    ...
    try {
      ...
      if (config?.api_key) {
        const [ragStats, memStats] = await Promise.all([
          apiClient.getRAGStats().catch(() => ({ document_count: 0, total_chunks: 0, total_size: 0 })),
          apiClient.getMemories(1).catch(() => ({ total: 0, memories: [] })),
        ]);
        setStats({
          documents: ragStats.document_count || 0,
